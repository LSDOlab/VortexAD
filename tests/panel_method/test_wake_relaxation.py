...
input_dict = {
    ...
    'reuse_AIC': False,
    # 'ref_chord': 0.649749105896,
    'ref_chord': 1.,

    # wake relaxation inputs:
    'wake_relaxation': True,
    'K': 0.1, # between 0.5 and 5
    'num_wake_planes': 16, # number of wake grid planes
    'num_wake_iterations': 3,
    'core_radius': 1.e-4,
...
x_w = outputs['x_w']
mu_w = outputs['mu_wake']
V_w_relax = outputs['V_w_relax']
...
inputs = [pitch]
outputs = [CL, CDi, CP]
outputs.extend([mu, x_w, mu_w])
outputs.append(V_w_relax)
...
# setting up dictionary for wake plotting
upper_TE_cells = panel_method.upper_TE_cells
lower_TE_cells = panel_method.lower_TE_cells
wake_connectivity = panel_method.wake_connectivity
TE_indices_zeroed = panel_method.TE_nodes_zeroed
edges2cells_w = panel_method.edges2cells_w

mu_wake = mu_val[:,upper_TE_cells] - mu_val[:,lower_TE_cells]
...
wake_plotting_dict = {
    'wake_mesh': x_w_val,
    'mu_wake': mu_w_val[0,:],
    'wake_connectivity': wake_connectivity,
    'wake_form': wake_form,
    'TE_indices_zeroed': TE_indices_zeroed,
    'wake_edges2cells': edges2cells_w,
    'plot_wake_color': True,
}

panel_method.plot(mu_val, wake_plot_params=wake_plotting_dict)
# panel_method.plot(mu_val, wake_plot_params=wake_plotting_dict, cmap='binary')
