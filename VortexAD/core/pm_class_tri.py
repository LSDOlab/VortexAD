...
from VortexAD.core.pm.steady_panel_solver_tri import steady_panel_solver
import os
...
class PanelMethodTri(object):
    ...
    def plot(self, data_to_plot, bounds=None, cmap='jet', camera=False, screenshot=False):
        ...
        '''
        from VortexAD.utils.plotting.plot_unstructured import plot_pressure_distribution
        plot_pressure_distribution(self.points_orig, data_to_plot, connectivity=self.cells, bounds=bounds, interactive=True, top_view=False, cmap=cmap, camera=camera, screenshot=screenshot)
        # cell_types = self.cells.keys()
        # num_cells = np.sum([len(self.cells[cell_type]) for cell_type in cell_types])
        # combined_cells = []
        # for cell_type in cell_types:
