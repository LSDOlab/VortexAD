...
def find_cell_adjacency(points, cells, radius=1.e-10):
    ...
    # combined_cells is a list of lists with each cell type combined

    # Finding the edge corresponding to a cell (edge tuple is the dictionary key)
    edges2cells = {}
    for c, cell in enumerate(combined_cells):
        cell = list(cell)
        cell.append(cell[0])
    ...
    cell_adjacency_dict = {i: [] for i in range(num_cells)}
    for edge in edges2cells.keys():
        cell_pairs = edges2cells[edge]
        if len(cell_pairs) < 2:
            continue # means this is an edge on the border of the mesh
        cell_adjacency_dict[cell_pairs[0]].append(cell_pairs[1])
        cell_adjacency_dict[cell_pairs[1]].append(cell_pairs[0])

    # cell_adjacency = {cell_type: {} for cell_type in cell_types}
    cell_adjacency = {cell_type: [] for cell_type in cell_types}
    for cell in cell_adjacency_dict.keys():
        adj_cells = cell_adjacency_dict[cell]
        if len(adj_cells) == 3:
            # cell_adjacency['triangle'][cell] = adj_cells
            cell_adjacency['triangle'].append(adj_cells)
        elif len(adj_cells) == 4:
            # cell_adjacency['quad'][cell] = adj_cells
            cell_adjacency['quad'].append(adj_cells)

    # cell_adjacency = np.array(list(cell_adjacency.values()), dtype=int)
    # # # # cell_adjacency = list(cell_adjacency.values())

    # Finding elements corresponding to each node/vertex --> ONLY USED IN HIGHER-ORDER METHOD
    ...
    while True:
        ...
        for cell_type in cell_types:
            for dup_ind in dup_ind_list:
                asdf = np.where(cells[cell_type] == dup_ind)
                row_ind, col_ind = list(asdf[0]), list(asdf[1])
                cells[cell_type][row_ind, col_ind] = npu

        # ==== adjusting cells for shifted points ====
        # inner loop is cell type bc the index shift relies 
        # on the number of duplicate points
        shift_ind = 0
        for dup_ind in dup_ind_list:
            for cell_type in cell_types:
                asdf = np.where(cells[cell_type] > dup_ind - shift_ind)
                row_ind, col_ind = list(asdf[0]), list(asdf[1])
                cells[cell_type][row_ind, col_ind] += -1
            shift_ind += 1
        # break
